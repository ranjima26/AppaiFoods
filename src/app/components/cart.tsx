                ...
                {items.map(({ product, quantity }) => (
                  <li key={product.slug} className="flex gap-4 rounded-3xl border border-[#003820]/10 bg-white p-4 shadow-sm sm:gap-6 sm:p-6">
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      <div className="flex items-start justify-between gap-2"><div><h2 className="text-xl sm:text-3xl"><Link href={`/shop/${product.slug}`}>{product.name}</Link></h2><p className="mt-1 text-xs text-[#607167] sm:text-sm">{product.subtitle}</p></div><button type="button" onClick={() => removeItem(product.slug)} aria-label={`Remove ${product.name} from cart`} className="grid size-10 shrink-0 place-items-center rounded-full text-[#79857d] hover:bg-red-50 hover:text-red-600"><FiTrash2 aria-hidden="true" /></button></div>
                      <p className="mt-2 text-xs text-[#607167]">{formatPrice(product.price)} each</p>
