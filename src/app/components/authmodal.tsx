...
export default function AuthModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    dialog?.showModal();
    // Lock both html and body to fully prevent background scrolling
  ...
  return createPortal(
    <dialog ref={dialogRef} data-lenis-prevent aria-labelledby="auth-title" onCancel={(event) => { event.preventDefault(); onClose(); }} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 m-auto max-h-[calc(100svh-2rem)] w-[calc(100%-2rem)] max-w-md overflow-x-hidden overflow-y-auto overscroll-contain rounded-[2rem] bg-white p-0 text-[#003820] shadow-2xl [scrollbar-width:none] backdrop:bg-[#002817]/65 backdrop:backdrop-blur-sm [&::-webkit-scrollbar]:hidden">
      <div>
